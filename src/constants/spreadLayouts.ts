...
export const spreadLayouts: Record<string, SpreadLayoutConfig> = {
  ...
  'cross-5-love2': {
    // Corresponds to l2: Finding New Love & user "LOVE SPREAD 2"
    // Layout:  C3
    //       C1 C5 C2
    //          C4
    containerClassName:
      'grid grid-cols-3 grid-rows-3 gap-x-6 gap-y-4 p-4 w-full max-w-2xl mx-auto items-center justify-center min-h-[600px]',
    cardPositions: [
  ...
  'square-4-project': {
    // Corresponds to p2: Project Success
    // Layout: C1 C2
    //         C3 C4
    containerClassName:
      'grid grid-cols-2 grid-rows-2 gap-6 p-4 w-full max-w-md mx-auto items-center justify-center min-h-[550px]',
    cardPositions: [
  ...
  'row-3-financial1': {
    // Corresponds to f1: Financial Snapshot & user "FINANTIAL SPREAD 2"
  ...
  'square-4-wealth': {
    // Corresponds to f2: Wealth Growth
    // Layout: C1 C2
    //         C3 C4
    containerClassName:
      'grid grid-cols-2 grid-rows-2 gap-6 p-4 w-full max-w-md mx-auto items-center justify-center min-h-[550px]',
...
export const getLayoutConfig = (layoutType: string | undefined, cardCount: number): SpreadLayoutConfig => {
  ...
  if (cardCount === 2) gridClasses += ` sm:grid-cols-2`;
    ...
    gridClasses += ` sm:grid-cols-3`; // Default to 3 cols on sm and up if not specified
