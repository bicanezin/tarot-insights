...
export default function HistoryPage() {
  ...
  const exportReadingToJson = (reading: SavedReading) => {
    const jsonString = `data:text/json;charset=utf-8,${encodeURIComponent(JSON.stringify(reading, null, 2))}`;
    const link = document.createElement("a");
    link.href = jsonString;
    link.download = `tarot-reading-${reading.userName.replace(/\s+/g, "_")}-${new Date(reading.readingDate)
      .toLocaleDateString()
      .replace(/\//g, "-")}.json`;
    link.click();
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {sortedReadings.length === 0 ? (
        <p className="text-center">{t("noReadingsFound")}</p>
      ) : (
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={() => handleDeleteReading(reading.id)}>{t("delete")}</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                </div>
